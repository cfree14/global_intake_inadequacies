...
g2 <- ggplot(ar_df, aes(x=ar, ymax=density, ymin=0)) +
  ...
  geom_vline(xintercept=params$ar, linetype="dotted") +
  # Plot param labels
  annotate(geom="text", x=xmax2, y=ymax2, label="Mean: Allen et al. 2022", color="black", hjust=1, size=2) +
  annotate(geom="text", x=xmax2, y=ymax2*0.92, label="CV: Renwick et al. 2004", color="grey50", hjust=1, size=2) +
  # Labels
...
# Plot risk curve
ymax3 <- 1
sev_label <- paste0(round(params$sev, digits = 1), "% of subpopulation")
...
g3 <- ggplot(ar_df, aes(x=ar, y=risk)) +
  ...
  # geom_line(data=dist_shift, mapping=aes(x=x, y=y_rel)) +
  # Requirement
  geom_vline(xintercept=params$ar, linetype="dotted") +
  # Plot param labels
  annotate(geom="text", x=xmax2, y=ymax3, label=sev_label, color="black", hjust=1, size=2) +
  annotate(geom="text", x=xmax2, y=ymax3*0.92, label=ppl_label, color="black", hjust=1, size=2) +
...
# Export
ggsave(g, filename=file.path(plotdir, "FigSX_conceptual_figure.png"),
