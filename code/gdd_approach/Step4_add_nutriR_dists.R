...
# Read data
data_orig <- readRDS(file=file.path(outdir, "2018_subnational_nutrient_intake_estimates.Rds"))

# Get distributions
dists_orig <- nutriR::dists_full

# Read GENuS dissimilarity matrix
diss_matrix <- readRDS(file=file.path(outdir, "GDD_country_dissimilarity.Rds"))
...
# Nutrient coverage
nutrients_gdd <- sort(unique(data_orig$nutrient))
nutrients_dist <- sort(unique(dists_orig$nutrient))
nutrients_gdd[!nutrients_gdd %in% nutrients_dist]

# Format dists
dists <- dists_orig %>%
  # Rename
  rename(age_range=age_group) %>%
  # Recode nutrients to match GDD data
  mutate(nutrient=recode(nutrient, "Thiamine"="Thiamin")) %>%
...
data <- data_orig %>%
  # Add distribution code
  mutate(dist_id=paste(nutrient, iso3, sex, age_range, sep="-"),
         dist_id_genus=paste(nutrient, gdd_iso3, sex, age_range, sep="-")) %>%
...
# Find most similar country with shape info
...
data1 <- data %>%
  ...
  left_join(sim_cntry_w_nutriR_key, by=c("gdd_iso3"="iso3", "nutrient"))
...
data2 <- data1 %>%
  ...
  rowwise() %>%
  mutate(dist_id_shape=ifelse(shape_source=="From closest age group",
...
data5_gamma <- data5 %>%
  ...
  # Shift parameters
  rowwise() %>%
  mutate(g_shape_shift=nutriR::shift_dist(shape=g_shape, rate=g_rate, to=supply_med, plot=F)$shape,
         g_rate_shift=nutriR::shift_dist(shape=g_shape, rate=g_rate, to=supply_med, plot=F)$rate) %>%
  ungroup() %>%
...
data5_lognormal <- data5 %>%
  ...
  # Shift parameters
  rowwise() %>%
  mutate(ln_meanlog_shift=nutriR::shift_dist(meanlog=ln_meanlog, sdlog=ln_sdlog, to=supply_med, plot=F)$meanlog,
         ln_sdlog_shift=nutriR::shift_dist(meanlog=ln_meanlog, sdlog=ln_sdlog, to=supply_med, plot=F)$sdlog) %>%
  ungroup() %>%
  # Calculate intake inadequacy
  rowwise() %>%
...
saveRDS(data6_simple, file.path(outdir, "2018_subnational_nutrient_intake_inadequacy_estimates_simple.Rds"))
