...
wbi <- wb_indicators()
wbi_pop <- wbi %>%
  filter(grepl("SP.POP", indicator_id))
...
# 1961-2021 population by country and age/sex
data_orig <- wb_data(country=wb_cntry_df$iso3c, indicator = age_sex_ids , start_date = 1950, end_date = 2021, return_wide = F)
